...
class Inquiries_model extends CI_Model {
    ...
    private $table_view = 'inquiries';
    private $column_order = array(null, 'name', 'email', 'contact', 'status', 'created_date', 'modified_date', null);
    private $column_search = array('name', 'email', 'contact', 'status', 'created_date', 'modified_date');
    ...
    private function _getTablesQuery($array = array()) {
        ...
        if ($this->input->post('inquiry_type_id')):
            $this->db->where('id IN(SELECT id FROM inquiry_to_types WHERE type_id=' . $this->input->post('inquiry_type_id') . ')', NULL);
        endif;
...
//        exit;
    ...
    public function setTypes($id) {
        $this->db->where('id', $id);
        $this->db->delete('inquiry_to_types');
        ...
        if ($this->input->post('types')):
            ...
            if ($types):
                foreach ($types as $value) :
                    ...
                    $this->db->insert('inquiry_to_types');
                endforeach;
            endif;
