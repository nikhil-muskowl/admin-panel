...
class My_leave_applications_api extends Restserver\Libraries\REST_Controller {
    ...
    public function __construct($config = 'rest') {
        ...
        $this->load->model('leave_managment_module/my_leave_applications_model');
    ...
    public function index_post() {
        ...
        $list = $this->my_leave_applications_model->getTables();
        ...
        $this->data['recordsTotal'] = $this->my_leave_applications_model->countAll();
        $this->data['recordsFiltered'] = $this->my_leave_applications_model->countFiltered();
    ...
    public function list_post() {
        ...
        foreach ($list as $object) :
            $action = '';
            $action .= '<a class="btn btn-sm btn-primary" href="' . base_url('leave_managment_module/my_leave_applications/form/' . $object['id']) . '" data-toggle="tooltip" title="' . $this->lang->line('text_edit') . '"><i class="fa fa-pencil"></i></a>';
    ...
    public function detail_get($id) {
        $this->data = array();
        $object = $this->my_leave_applications_model->getById($id);
        if ($object):
            $result = array(
                'id' => $object['id'],
                'user_id' => $object['user_id'],
    ...
    public function save_post() {
        $this->data = array();
        $this->_validation();
        $result = $this->my_leave_applications_model->postData();
        if ($result):
            $this->data['status'] = TRUE;
    ...
    public function _validation() {
        ...
        $this->form_validation->set_rules('total', 'total', 'callback_check_total_leave');
    ...
    public function check_total_leave() {
        ...
        $minDays = $this->settings_lib->dateToDay($current_date, $from_date);
//        print_r($minDays);
//        exit;
        ...
        if ($total > 6) :
            ...
            return FALSE;
        elseif ($minDays < 3):
            $this->form_validation->set_message('check_total_leave', 'you must apply leave before 3 days!');
            return FALSE;
        else :
            return TRUE;
        endif;
    ...
    public function delete_get($id) {
        $this->data = array();
        $result = $this->my_leave_applications_model->deleteById($id);
        if ($result):
            $this->data['status'] = TRUE;
            $this->data['message'] = $this->lang->line('text_submit_success');
        else:
    ...
    public function dateToDay_post() {
        ...
        $leave_types = $this->my_leave_applications_model->getTypeById($this->input->post('leave_type_id'));
        if ($leave_types):
    ...
    public function leave_status_post() {
        ...
        $result = $this->my_leave_applications_model->leave_status();
        if ($result):
            $this->data['status'] = TRUE;
            $this->data['message'] = $this->lang->line('text_submit_success');
        else:
    ...
    public function preview_get($id) {
        $this->data = array();
        $result = '';
        $result = $this->my_leave_applications_model->getPreview($id);
        if ($result):
            $this->data['status'] = TRUE;
    ...
    public function send_get($id) {
        $this->data = array();
        $result = $this->my_leave_applications_model->sendMail($id);
        if ($result):
            $this->data['status'] = TRUE;
            $this->data['message'] = $this->lang->line('text_submit_success');
        else:
