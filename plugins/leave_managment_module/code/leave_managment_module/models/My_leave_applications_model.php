...
class My_leave_applications_model extends CI_Model {
...
//        exit;
    ...
    public function getUserLeaveAuthorities($id) {
        $this->db->from('user_authorities_view');
    ...
    public function sendMail($id) {
        ...
        if ($data):
            ...
            $user_authorities = $this->getUserLeaveAuthorities($data['user_id']);
            $toEmail = '';
            $cc = array();
            if ($user_authorities):
                foreach ($user_authorities as $key => $user_leave_authority) :
                    if ($key == 0):
