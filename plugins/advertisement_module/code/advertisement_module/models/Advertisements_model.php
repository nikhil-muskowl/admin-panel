...
class Advertisements_model extends CI_Model {
    ...
    private $detailTable = 'advertisement_details';
    private $table_view = 'advertisements_view';
    ...
    private function _getTablesQuery($array = array()) {
        ...
        if ($this->input->post('advertisement_type_id')):
            $this->db->where('id IN(SELECT advertisement_id FROM advertisement_to_types WHERE advertisement_type_id=' . $this->input->post('advertisement_type_id') . ')', NULL);
        endif;
...
//        exit;
    ...
    public function postImages($advertisement_id) {
        ...
        if ($this->input->post('images')):
            foreach ($this->input->post('images') as $value) :
                $this->db->set('advertisement_id', $advertisement_id);
    ...
    public function images($id) {
        $result = array();
        $this->db->from('advertisement_images');
    ...
    public function setTypes($id) {
        $this->db->where('advertisement_id', $id);
        $this->db->delete('advertisement_to_types');

        if ($this->input->post('types')):
            foreach ($this->input->post('types') as $value) :
                $this->db->set('advertisement_id', $id);
                $this->db->set('advertisement_type_id', $value);
                $this->db->insert('advertisement_to_types');
            endforeach;
        endif;
    }
    ...
    public function getTypes($id) {
        $result = array();
        $this->db->from('advertisement_to_types');
        $this->db->where('advertisement_id', $id);
        $query = $this->db->get();
        $description = $query->result_array();
        ...
        if ($description):
            foreach ($description as $value) :
                $result[] = $value['advertisement_type_id'];
            endforeach;
        endif;
     ...
     public function getTypesNames($id) {
        ...
        $this->db->from('advertisement_to_types stt');
        $this->db->join('advertisement_types_view stv', 'stv.id=stt.advertisement_type_id');
        $this->db->where('stt.advertisement_id', $id);
    ...
    public function setTags($id) {
        $this->db->where('advertisement_id', $id);
        $this->db->delete('advertisement_tags');

        if ($this->input->post('tags')):
            foreach ($this->input->post('tags') as $value) :
                $this->db->set('advertisement_id', $id);
                $this->db->set('tag', $value);
                $this->db->insert('advertisement_tags');
            endforeach;
        endif;
    }
    ...
    public function getTags($id) {
        $result = array();
        $this->db->from('advertisement_tags');
        $this->db->where('advertisement_id', $id);
        $query = $this->db->get();
        $description = $query->result_array();
    ...
    public function unsetUrlAlias($id) {
        ...
        $this->db->where('type', 'advertisements');
    ...
    public function setUrlAlias($id) {
        ...
        if ($this->input->post('url_alias')):
            foreach ($this->input->post('url_alias') as $key => $value) :
                ...
                $this->db->set('type', 'advertisements');
