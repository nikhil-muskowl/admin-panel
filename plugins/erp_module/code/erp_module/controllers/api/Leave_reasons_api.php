...
class Leave_reasons_api extends Restserver\Libraries\REST_Controller {
    ...
    public function index_post() {
        ...
        $list = $this->leave_reasons_model->getTables();
        ...
        $this->data['recordsTotal'] = $this->leave_reasons_model->countAll();
        $this->data['recordsFiltered'] = $this->leave_reasons_model->countFiltered();
    ...
    public function list_post() {
        ...
        foreach ($list as $object) :
            $action = '';
            $action .= '<a class="btn btn-sm btn-primary" href="' . base_url('leave_managment_module/leave_reasons/form/' . $object['id']) . '" data-toggle="tooltip" title="' . $this->lang->line('text_edit') . '"><i class="fa fa-pencil"></i></a>';
    ...
    public function detail_get($id) {
        $this->data = array();
        $object = $this->leave_reasons_model->getById($id);
        if ($object):
            $result[] = array(
                'id' => $object['id'],
                'title' => $object['title'],                
    ...
    public function save_post() {
        $this->data = array();
        $this->_validation();
        $result = $this->leave_reasons_model->postData();
        if ($result):
            $this->data['status'] = TRUE;
    ...
    public function delete_get($id) {
        $this->data = array();
        $result = $this->leave_reasons_model->deleteById($id);
        if ($result):
            $this->data['status'] = TRUE;
            $this->data['message'] = $this->lang->line('text_submit_success');
        else:
