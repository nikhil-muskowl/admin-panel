                    ...
                    <table id="table" class="table table-striped table-bordered" cellspacing="0" width="100%">
                        <thead>
                            <tr>
                                ...
                                <th><?= humanize('player_name') ?></th>
                                <th><?= humanize('points') ?></th>                                
                                <th><?= humanize('approved_status') ?></th>
...
<script type="text/javascript">
    ...
    function approved_poitns(id) {
        if (confirm('are you sure want to approved this data?')) {
            $.ajax({
                url: '<?= $ajax_approved_poitns ?>' + id,
                type: "GET",
                dataType: "JSON",
                success: function (data) {
                    if (data.status) {
    ...
    function cancel_poitns(id) {
        if (confirm('are you sure want to cancel this data?')) {
            $.ajax({
                url: '<?= $ajax_cancel_poitns ?>' + id,
                type: "GET",
                dataType: "JSON",
                success: function (data) {
                    if (data.status) {
    ...
    function return_poitns(id) {
        if (confirm('are you sure want to cancel this data?')) {
            $.ajax({
                url: '<?= $ajax_return_poitns ?>' + id,
                type: "GET",
                dataType: "JSON",
                success: function (data) {
                    if (data.status) {
