...
class Cricket_matches_api extends Restserver\Libraries\REST_Controller {
    ...
    public function __construct($config = 'rest') {
        ...
        $this->load->model('cricket_module/cricket_matches_model');
    ...
    public function index_post() {
        ...
        $list = $this->cricket_matches_model->getTables();
        ...
        foreach ($list as $object) :
            $result[] = array(
                ...
                'start_date' => date('Y-m-d s:i A', strtotime($object['start_date'])),
                'end_date' => date('Y-m-d s:i A', strtotime($object['end_date'])),
        ...
        $this->data['recordsTotal'] = $this->cricket_matches_model->countAll();
        $this->data['recordsFiltered'] = $this->cricket_matches_model->countFiltered();
    ...
    public function list_post() {
        ...
        foreach ($list as $object) :
            $action = '';
            $action .= '<a class="btn btn-sm btn-primary" href="' . base_url('cricket_module/cricket_matches/form/' . $object['id']) . '" data-toggle="tooltip" title="Edit"><i class="fa fa-pencil"></i></a>';
            ...
            $result[] = array(
                $checkbox,
                $object['name'],
                date('Y-m-d s:i A', strtotime($object['start_date'])),
                date('Y-m-d s:i A', strtotime($object['end_date'])),
    ...
    public function detail_get($id) {
        $this->data = array();
        $result = $this->cricket_matches_model->getById($id);
        if ($result):
    ...
    public function save_post() {
        $this->data = array();
        $this->_validation();
        $result = $this->cricket_matches_model->postData();
        if ($result):
            $this->data['status'] = TRUE;
    ...
    public function delete_get($id) {
        $this->data = array();
        $result = $this->cricket_matches_model->deleteById($id);
        if ($result) {
            $this->data['status'] = TRUE;
