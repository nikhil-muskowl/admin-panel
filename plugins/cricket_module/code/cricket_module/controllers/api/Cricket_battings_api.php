...
class Cricket_battings_api extends Restserver\Libraries\REST_Controller {
    ...
    public function index_post() {
        ...
        $list = $this->cricket_battings_model->getTables();

        $result = array();
        foreach ($list as $object) :
            $result[] = array(
                'id' => $object['id'],
                'match_name' => $object['match_name'],
        ...
        $this->data['recordsTotal'] = $this->cricket_battings_model->countAll();
        $this->data['recordsFiltered'] = $this->cricket_battings_model->countFiltered();
    ...
    public function list_post() {
        ...
        foreach ($list as $object) :
            $action = '';
            $action .= '<a class="btn btn-sm btn-primary" href="' . base_url('cricket_module/cricket_battings/form/' . $object['id']) . '" data-toggle="tooltip" title="Edit"><i class="fa fa-pencil"></i></a>';
    ...
    public function save_post() {
        $this->data = array();
        $this->_validation();
        $result = $this->cricket_battings_model->postData();
        if ($result):
            $this->data['status'] = TRUE;
    ...
    public function _validation() {
        ...
        if ($this->form_validation->run() == FALSE):
            if (form_error('match_id', '', '')):
                $this->error[] = array(
                    'id' => 'match_id',
                    'text' => form_error('match_id', '', '')
                );
            endif;
    ...
    public function delete_get($id) {
        $this->data = array();
        $result = $this->cricket_battings_model->deleteById($id);
        if ($result) {
            $this->data['status'] = TRUE;
