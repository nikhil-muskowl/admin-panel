...
class Carts_api extends Restserver\Libraries\REST_Controller {
    ...
    public function list_post() {
        ...
        foreach ($list as $object) :
            $action = '';
            $action .= '<a class="btn btn-sm btn-primary" href="' . base_url('order_module/carts/form/' . $object['id']) . '" data-toggle="tooltip" title="Edit"><i class="fa fa-pencil"></i></a>';
    ...
    public function save_post() {
        $this->data = array();
        $this->_validation();
        $result = $this->carts_model->postData();
        if ($result):
            $this->data['status'] = TRUE;
    ...
    public function _validation() {
        ...
        $this->form_validation->set_rules('user_id', 'user', 'required|greater_than[0]');
    ...
    public function edit_post() {
        ...
        $result = $this->carts_model->editData();
        if ($result):
            $this->data['status'] = TRUE;
            $this->data['message'] = 'update success!';
            $this->data['result'] = $result;
    ...
    public function _editValidation() {
        $this->data = array();

        $this->form_validation->set_rules('id', 'id', 'required|greater_than[0]');
        $this->form_validation->set_rules('quantity', 'quantity', 'required|greater_than[0]');
    ...
    public function delete_get($id) {
        $this->data = array();
        $result = $this->carts_model->deleteById($id);
        if ($result) {
            $this->data['status'] = TRUE;
