...
class User_leave_authorities_api extends Restserver\Libraries\REST_Controller {
    ...
    public function index_post() {
        ...
        $list = $this->user_leave_authorities_model->getTables();
        ...
        $this->data['recordsTotal'] = $this->user_leave_authorities_model->countAll();
        $this->data['recordsFiltered'] = $this->user_leave_authorities_model->countFiltered();
    ...
    public function list_post() {
        ...
        foreach ($list as $object) :
            $action = '';
            $action .= '<a class="btn btn-sm btn-primary" href="' . base_url('leave_managment_module/user_leave_authorities/form/' . $object['id']) . '" data-toggle="tooltip" title="' . $this->lang->line('text_edit') . '"><i class="fa fa-pencil"></i></a>';
    ...
    public function detail_get($id) {
        $this->data = array();
        $object = $this->user_leave_authorities_model->getById($id);
        if ($object):
            $result[] = array(
                'id' => $object['id'],
                'user_name' => $object['user_name'],
    ...
    public function save_post() {
        $this->data = array();
        $this->_validation();
        $result = $this->user_leave_authorities_model->postData();
        if ($result):
            $this->data['status'] = TRUE;
    ...
    public function delete_get($id) {
        $this->data = array();
        $result = $this->user_leave_authorities_model->deleteById($id);
        if ($result):
            $this->data['status'] = TRUE;
            $this->data['message'] = $this->lang->line('text_submit_success');
        else:
