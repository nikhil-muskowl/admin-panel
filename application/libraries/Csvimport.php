...
//                    $result[$new_row][$value] = utf8_encode(trim($data[$key]));
                    $result[$new_row][$value] = mb_convert_encoding($data[$key], 'UTF-8', 'auto');
                }
            }
    ...
    private function _set_filepath($filepath) {
        file_put_contents($filepath, str_replace("\t", ",", iconv('UTF-16', 'UTF-8', file_get_contents($filepath))));
    ...
    private function _delete_filepath() {
