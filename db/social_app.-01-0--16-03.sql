...
INSERT INTO `events` VALUES (1,2,'2018-11-22 01:00:00','2018-11-23 03:00:00','','upload/water.png','','0.000000','0.000000',1,0,0,'2018-11-22 11:42:22','2018-11-22 12:19:39');
...
INSERT INTO `story_to_types` VALUES (1,1,0),(1,6,0),(2,3,0),(3,2,0),(3,1,0),(4,3,0),(5,6,0),(5,5,0),(6,3,0),(6,2,0),(6,4,0),(7,3,0),(7,5,0),(8,3,0),(8,4,0),(9,3,0),(9,2,0),(9,5,0),(10,2,0),(10,5,0),(11,3,0),(12,3,0),(12,2,0),(13,4,0),(14,3,0),(15,3,0),(16,3,0),(17,3,0);
...
/*!50001 VIEW `events_view` AS select `t`.`id` AS `id`,`t`.`user_id` AS `user_id`,`t`.`from_date` AS `from_date`,`t`.`to_date` AS `to_date`,`t`.`banner` AS `banner`,`t`.`image` AS `image`,`t`.`location` AS `location`,`t`.`latitude` AS `latitude`,`t`.`longitude` AS `longitude`,`t`.`status` AS `status`,`t`.`created_by` AS `created_by`,`t`.`modified_by` AS `modified_by`,`t`.`created_date` AS `created_date`,`t`.`modified_date` AS `modified_date`,`td`.`language_id` AS `language_id`,`td`.`title` AS `title`,`u`.`name` AS `user_name`,`u`.`image` AS `user_image` from ((`events` `t` left join `event_details` `td` on((`td`.`id` = `t`.`id`))) left join `users` `u` on((`u`.`id` = `t`.`user_id`))) */;
...
-- Dump completed on 2018-11-22 16:03:46
